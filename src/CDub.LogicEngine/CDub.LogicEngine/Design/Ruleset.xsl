...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="condition">
		...
		<xsl:variable name="notOp">
			<xsl:value-of select="./@not = 'y' or ./@not = '1'"/>
		</xsl:variable>
	...
	<xsl:template name="translateOperator">
		...
		<xsl:choose>
			<xsl:when test="$op = 'eq' and $notOp">!=</xsl:when>
			<xsl:when test="$op = 'eq'">==</xsl:when>
			<xsl:when test="$op = 'lt' and $notOp">&gt;=</xsl:when>
			<xsl:when test="$op = 'lt'">&lt;</xsl:when>
			<xsl:when test="$op = 'gt' and $notOp">&lt;=</xsl:when>
			<xsl:when test="$op = 'gt'">&gt;</xsl:when>
			<xsl:when test="$op = 'ct' and $notOp">!contains</xsl:when>
			<xsl:when test="$op = 'ct'">contains</xsl:when>
			<xsl:when test="$op = 'sw' and $notOp">!starts_with</xsl:when>
			<xsl:when test="$op = 'sw'">starts_with</xsl:when>
			<xsl:when test="$op = 'ew' and $notOp">!ends_with</xsl:when>
